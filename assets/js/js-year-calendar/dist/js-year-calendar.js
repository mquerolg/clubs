...
// todo
              if (monthData.length > 0) {
                var iter=0;
                var iter2=0;
                month.querySelectorAll('.day-content').forEach(function (day, index,arr) {
                  ...
                  if ((_this.options.minDate == null || currentDate >= _this.options.minDate) && (_this.options.maxDate == null || currentDate <= _this.options.maxDate)) {
                    ...
                    if (dayData.length > 0 && (_this.options.displayDisabledDataSource || !_this._isDisabled(currentDate))) {
                      iter = iter+1;
                      _this._renderDataSourceDay(day, currentDate, dayData,iter,iter2,arr,monthData);
                    }
                  }
                });
              }
      ...
      value: function _renderDataSourceDay(elt, currentDate, events,index,index2,arr, monthData) {
          ...
          case 'border':
              var boxShadow = '';
                ...
                for (var i = 0; i < events.length; i++) {
                  ...
                  boxShadow += "inset 0 40px 0 0 var(--table-title-color)";
                ...
                if(arr.length>0){
                  if(events[0]["startDate"].toISOString().slice(0, 10) == currentDate.toISOString().slice(0, 10)){
                    parent.style.borderStartStartRadius = "15px";
                    parent.style.borderEndStartRadius = "15px";
                  }
                  if(events[0]["endDate"].toISOString().slice(0, 10) == currentDate.toISOString().slice(0, 10)){
                    parent.style.borderStartEndRadius = "15px";
                    parent.style.borderEndEndRadius = "15px";
                  }
                }

              for (var i = 0; i < monthData.length; i++) {
                if(events[0]["endDate"].toISOString().slice(0, 10) == events[0]["startDate"].toISOString().slice(0, 10)){
                
                  parent.style.borderRadius = "70%";
                  parent.style.boxShadow = "inset 0 0px 1px 2px var(--table-title-color)";
