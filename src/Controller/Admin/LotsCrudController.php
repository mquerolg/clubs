...
class LotsCrudController extends AdminCrudController
{
    ...
    */
    ...
    public function handleStatusChange(Lots $lot, int $newStatusId)
    {

        $entityManager = $this->container->get('doctrine')->getManager();

        // Obtén el estado actual directamente desde la base de datos
        $currentStatusId = $entityManager->getConnection()->fetchOne(
            'SELECT status_id FROM CLUBS_LOTS WHERE id = :id',
            ['id' => $lot->getId()]
        );

         // Registra el estado actual en el log
        $this->logger->info(sprintf(
        '(2)Before status change: Lot "%s" has status_id: %d',
        $lot->getTitle(),
        $currentStatusId
        ));

        // Actualiza el estado del lote
        $newStatus = $entityManager->getRepository(Status::class)->find($newStatusId);
        ...
        $this->logger->info(sprintf(
            '(2)After status change: Lot "%s" updated to status_id: %d',
            $lot->getTitle(),
            $newStatusId
        ));

        // Si el estado cambió de 1 a 2, envía el correo
        if ($currentStatusId == 1 && $newStatusId == 2) {
            SamcService::notifyAdminLotChangeStatus([
                'TITLE' => $lot->getTitle(),
                'AUTHORSHIP' => $lot->getAuthorship(),
                'WAREHOUSE' => $lot->getWarehouse(),
            ]);
        }

        $this->logger->info(sprintf(
            'Correo enviado: El lote "%s" (Autor: %s, Almacén: %s) ha cambiado a estado RESERVADO.',
            $lot->getTitle(),
            $lot->getAuthorship(),
            $lot->getWarehouse()
        ));
    ...
    */
    ...
    public function configureAdminIndexFields(): iterable
    {
        ...
        // $lots = $this->container->get('doctrine')->getRepository(Lots::class)->findAll();
        // $lotIds = array_map(fn($lot) => $lot->getId(), $lots);
        // $this->logger->info('IDs de todos los lotes: ' . implode(', ', $lotIds));
