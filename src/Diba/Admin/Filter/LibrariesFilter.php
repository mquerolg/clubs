...
class LibrariesFilter implements FilterInterface
{
    use FilterTrait;

    public static function new(QueryBuilder $qb, string $propertyName, $label = null): self
    {
        $rows = $qb->select('libraries.name')
            ->from(Libraries::class, 'libraries')
            ->distinct('libraries.name')
    ...
    public function apply(QueryBuilder $queryBuilder, FilterDataDto $filterDataDto, ?FieldDto $fieldDto, EntityDto $entityDto): void
    {
        ...
        if (null !== $value && is_countable($value) && 0 !== \count($value)) {
            ...
            if ($entityDto->getFqcn() == "App\Entity\Libraries") {
                $orX->add(sprintf('entity.name %s (:%s)', $comparison, $parameterName));

                if (ComparisonType::NEQ === $comparison) {
                    $orX->add(sprintf('entity.name IS NULL'));
                }
            } else {
                $orX->add(sprintf('library.name %s (:%s)', $comparison, $parameterName));

                if (ComparisonType::NEQ === $comparison) {
                    $orX->add(sprintf('library.name IS NULL'));
                }
            }
