...
class ZoneFilter implements FilterInterface
{
    ...
    {
        $rows = $qb->select('zones.zone')
            ->from(Zones::class, 'zones')
            ->distinct('zones.zone')
            ->orderBy('zones.zone', 'ASC')
            ->getQuery()->getResult();

        foreach ($rows as $value) {
            $zones[current($value)] = current($value);
        }
    ...
    public function apply(QueryBuilder $queryBuilder, FilterDataDto $filterDataDto, ?FieldDto $fieldDto, EntityDto $entityDto): void
    {
        ...
        if (null !== $value && is_countable($value) && 0 !== \count($value)) {
            ...
            if ($entityDto->getFqcn() == "App\Entity\Libraries") {
                $orX->add(sprintf('entity.zone %s (:%s)', $comparison, $parameterName));

                if (ComparisonType::NEQ === $comparison) {
                    $orX->add(sprintf('entity.zone IS NULL'));
                }
            } else {
                $orX->add(sprintf('library.zone %s (:%s)', $comparison, $parameterName));

                if (ComparisonType::NEQ === $comparison) {
                    $orX->add(sprintf('library.zone IS NULL'));
                }
            }
