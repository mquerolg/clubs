...
class GenreFilter implements FilterInterface
{
    ...
    {
        $rows = $qb->select('genres.id, genres.name')
            ->from(Genres::class, 'genres')
    ...
    public function apply(QueryBuilder $queryBuilder, FilterDataDto $filterDataDto, ?FieldDto $fieldDto, EntityDto $entityDto): void
    {
        ...
        if (null !== $value && is_countable($value) && 0 !== \count($value)) {
            $orX = new Orx();
            $orX->add(sprintf('lot.genreId %s (:%s)', $comparison, $parameterName));

            if (ComparisonType::NEQ === $comparison) {
                $orX->add(sprintf('lot.genreId IS NULL'));
            }
